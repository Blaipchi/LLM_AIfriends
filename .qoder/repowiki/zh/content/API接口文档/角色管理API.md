...
## 简介
本文件为“角色管理”模块的完整API文档，覆盖AI角色的创建、更新、删除与单条查询接口。文档从接口定义、请求/响应格式、业务逻辑、数据模型、文件上传与媒体资源管理、错误处理与调试等方面进行系统化说明，并提供流程图与时序图帮助理解。
...
G["web/views/utils/photo.py<br/>旧图清理工具"]
H["web/urls.py<br/>路由注册"]
I["backend/settings.py<br/>媒体路径与JWT配置"]
end
subgraph "前端"
J["CreateCharacter.vue<br/>创建页面"]
K["UpdateCharacter.vue<br/>更新页面"]
L["Photo.vue<br/>头像裁剪组件"]
M["BackgroundImage.vue<br/>背景图裁剪组件"]
N["api.js<br/>HTTP客户端(带鉴权/刷新)"]
O["base64_to_file.js<br/>Base64转File工具"]
end
J --> N
K --> N
J --> L
...
图表来源
- [backend/web/urls.py:24-27](file://backend/web/urls.py#L24-L27)
...
## 结论
角色管理API提供了完整的CRUD能力，配合前端裁剪与文件转换，实现了良好的用户体验。建议后续增强异常类型与序列化校验，完善分页与批量操作能力，并加强媒体资源的版本与压缩策略以提升性能与稳定性。
